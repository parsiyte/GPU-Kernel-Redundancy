...
struct RMTDevice : public ModulePass {
  ...
  Function *createMajorityFuncton(Module &M,
    ...
    FunctionCallee GetBlockDim = M.getOrInsertFunction(
        "llvm.nvvm.read.ptx.sreg.ntid.x", GetThread.getFunctionType());
    ...
    BasicBlock *IfBlock = BasicBlock::Create(C, "If", MajorityVotingFunction);
    ...
    if (TIDthElementOfDeviceC->getType()->isFloatTy() ||
        TIDthElementOfDeviceB->getType()->isDoubleTy())
      DeviceADeviceBCMP =
          Builder.CreateFCmpOEQ(TIDthElementOfDeviceC, TIDthElementOfDeviceB);
    else
      DeviceADeviceBCMP =
          Builder.CreateICmpEQ(TIDthElementOfDeviceC, TIDthElementOfDeviceB);
  ...
  bool runOnModule(Module &M) override {
    ...
    for (unsigned int Index = 0; Index < Annotations->getNumOperands();
         Index++) {
      ...
      if (cast<MDString>(Feature)->getString() == "kernel") {
        ...
        if (Kernel->getName().contains("Revisited") ||
            Kernel->getName().contains("majorityVoting15") ||
            !PossibleOutputType->isPointerTy())
          continue;

        AttributeList Attributes = Kernel->getAttributes();
        errs() << Kernel
                      ->getFnAttribute("correctly-rounded-divide-sqrt-fp-math")
                      .getAsString()
               << "*\n";
        ...
        for (int OutputIndex = 0; OutputIndex < NumberOfReplication - 1;
             OutputIndex++) {
        ...
        FunctionType *NewKernelType =
            FunctionType::get(VoidType, NewFunctionType, true);
        std::string NewKernelName = Kernel->getName().str() + "Revisited";

        FunctionCallee NewKernelAsCallee =
            M.getOrInsertFunction(NewKernelName, NewKernelType);
        Function *NewKernelFunction =
            cast<Function>(NewKernelAsCallee.getCallee());
        ...
        AllocaInst *OutputAllocation =
            dyn_cast<AllocaInst>(OutputStore->getPointerOperand());
        ...
        Instruction *FirstRedundant =
            Builder.CreateAlloca(PossibleOutputType, nullptr, "FirstRedundant");
        Instruction *SecondRedundant = Builder.CreateAlloca(
            PossibleOutputType, nullptr, "SecondRedundant");
        Value *OriginalXAddr =
            Builder.CreateAlloca(Int32Type, nullptr, "OriginalX.addr");
        Value *OriginalYAddr =
            Builder.CreateAlloca(Int32Type, nullptr, "OriginalY.addr");
        ...
        // IRBuilder<> Builder(OutputAllocation->getNextNode());
        Instruction *TempRedundant =
            Builder.CreateAlloca(PossibleOutputType, nullptr, "TempRedundant");
        OutputAllocation->replaceAllUsesWith(TempRedundant);

        Builder.CreateStore(Output, OutputAllocation);

        Instruction *ThreadIDaddr =
            Builder.CreateAlloca(Int32Type, nullptr, "ThreadIDaddr");
        Instruction *NewThreadId =
            Builder.CreateAlloca(Int32Type, nullptr, "NewThreadID");
        Instruction *BlockIdaddrY =
            Builder.CreateAlloca(Int32Type, nullptr, "BlockIdaddrY");

        Value *BlockIDYCall;
        Value *BlockYID2;
        //Value *ThreadIDXCall = Builder.CreateCall(ThreadIDX);
        /*
        if(BlockIDY != nullptr){
        ...
        Instruction* Stored = Builder.CreateStore(NewThreadID, CalculatedThreadID->getPointerOperand());

        //CalculatedThreadID->getValueOperand()->replaceAllUsesWith(Builder.CreateLoad(NewThreadId));
  /*
                    Function *a;
            Value *b;
            Value *c;
            ...
            for (Module::iterator F = M.begin(); F != M.end(); ++F) {
              for (Function::iterator fi = F->begin(); fi != F->end(); ++fi) {
                for (BasicBlock::iterator bi = fi->begin(); bi != fi->end();
           ++bi) { if (CallInst *callInstruction = dyn_cast<CallInst>(bi)) { if
           (callInstruction->getCalledFunction()->getName() == "vprintf") {
                      // errs() << callInstruction->getNumArgOperands() <<
           "+++++\n"; a = callInstruction->getCalledFunction(); b =
           callInstruction->getArgOperand(0); c =
           callInstruction->getArgOperand(1);
            ...
            AllocaInst *PrintFAlloca = Builder.CreateAlloca(d->getScalarType());
             auto *XX =  Builder.CreateInBoundsGEP(PrintFAlloca,
           {ConstantInt::get(Type::getInt32Ty(Context),0),ConstantInt::get(Type::getInt32Ty(Context),0)});
             Builder.CreateStore(ThreadID, XX);
                Builder.CreateCall(
...
*/
     ...
     //   changeXID(NewKernelFunction, NewThreadId, Builder);
        ...
        SplitBlockAndInsertIfThenElse(ZeroCmp, ZeroCmp->getNextNode(),
                                      &ThenTerm, &FirstElseIfCondTerm);
        Builder.SetInsertPoint(ThenTerm);
        Builder.CreateStore(Builder.CreateLoad(OutputAllocation),
                            TempRedundant);
        ...
        OutputIdentifier = Builder.CreateLoad(ThreadIDaddr);
        Instruction *OneCmp =
            dyn_cast<Instruction>(Builder.CreateICmpEQ(OutputIdentifier, One32Bit));
        SplitBlockAndInsertIfThenElse(OneCmp, OneCmp->getNextNode(),
                                      &ElseIfTerm, &SecondElseTerm);
        ...
        OutputIdentifier = Builder.CreateLoad(ThreadIDaddr);
        Instruction *TwoCmp =
            dyn_cast<Instruction>(Builder.CreateICmpEQ(OutputIdentifier, Two32Bit));
        Instruction *NewBranch =
            SplitBlockAndInsertIfThen(TwoCmp, TwoCmp->getNextNode(), false);
        ...
        MDNode *TempN = MDNode::get(
            Context, ConstantAsMetadata::get(ConstantInt::get(Int32Type, 1)));
        MDNode *Con = MDNode::concatenate(N, TempN);
        Annotations->addOperand(MDNode::concatenate(
            MDNode::get(Context, ValueAsMetadata::get(NewKernelFunction)),
            Con));
        ...
        FunctionCallee MajorityVotingCallee = M.getOrInsertFunction(
            ...
            Type::getFloatPtrTy(Context), Type::getInt64Ty(Context)
...
struct RMTHost : public ModulePass {
  ...
  Function *createMajorityVoting(Module &M,
    ...
    std::string MajorityVotingFunctionName =
        "majorityVoting" +
        std::to_string(
            MajorityVotingPointerType->getPointerElementType()->getTypeID());
    ...
    for (BasicBlock::iterator CurrentInstruction = BB->begin();
      ...
      if (CallInst *FunctionCall = dyn_cast<CallInst>(CurrentInstruction)) {
        ...
        if (FunctionName == "cudaConfigureCall") {
          LoadInst *GridLoadInstr = dyn_cast_or_null<LoadInst>(
              FunctionCall->getArgOperand(2)); // 0 Grid X oluyor

          GetElementPtrInst *GEPS = dyn_cast_or_null<GetElementPtrInst>(
              GridLoadInstr
                  ->getPointerOperand()); // Grid {i64, i32} şekilde bir struct.
          AllocaInst *GridAlloca = dyn_cast_or_null<AllocaInst>(
              GEPS->getPointerOperand()); // Grid'in alloca instruction'ı
          for (Use &U : GridAlloca->uses()) {
            ...
            if (Casted != nullptr) {
              ...
              BitCastInst *CopySrc = dyn_cast_or_null<BitCastInst>(
                  GridMemoryCopy->getArgOperand(1));
              AllocaInst *GridTotalAllocation =
                  dyn_cast_or_null<AllocaInst>(CopySrc->getOperand(0));
              errs() << *GridTotalAllocation << "\n";
              bool Found = false;
              for (Function::iterator BasicB = MainFunction->begin();
                   BasicB != MainFunction->end(); BasicB++)
                for (BasicBlock::iterator CurrentInstruction2 = BasicB->begin();
                     CurrentInstruction2 != BasicB->end();
                     ++CurrentInstruction2) {
                  if (MemCpyInst *TempMemCpy =
                          dyn_cast<MemCpyInst>(CurrentInstruction2)) {
                    BitCastInst *PossibleGridX = dyn_cast_or_null<BitCastInst>(
                        TempMemCpy->getArgOperand(0));
                    AllocaInst *PossibleGridAllocation =
                        dyn_cast_or_null<AllocaInst>(
                            PossibleGridX->getOperand(0));
                    if (PossibleGridAllocation == GridTotalAllocation) {
                      BitCastInst *CopyDestination =
                          dyn_cast_or_null<BitCastInst>(
                              TempMemCpy->getArgOperand(1));
                      AllocaInst *DestinationAllocation =
                          dyn_cast_or_null<AllocaInst>(
                              CopyDestination->getOperand(0));
                      for (Function::iterator BasicB1 = MainFunction->begin();
                           BasicB1 != MainFunction->end(); BasicB1++)
                        for (BasicBlock::iterator CurrentInstruction3 =
                                 BasicB1->begin();
                             CurrentInstruction3 != BasicB1->end();
                             ++CurrentInstruction3) {
                          if (CallInst *FuncCall =
                                  dyn_cast<CallInst>(CurrentInstruction3)) {
                            StringRef FunctionName =
                                FuncCall->getCalledFunction()->getName();
                            if (FunctionName.contains("dim3") == true) {
                              errs() << *FuncCall << "\n";
                              if (FuncCall->getArgOperand(0) ==
  ...
  Value *findThreadY(BasicBlock *BB) {
    ...
    for (BasicBlock::iterator CurrentInstruction = BB->begin();
      ...
      if (CallInst *FunctionCall = dyn_cast<CallInst>(CurrentInstruction)) {
        ...
        if (FunctionName == "cudaConfigureCall") {
          LoadInst *GridLoadInstr = dyn_cast_or_null<LoadInst>(
              FunctionCall->getArgOperand(3)); // 0 Grid X oluyor
  ...
  void reMemCpy(IRBuilder<> Builder, std::string VariableName,
    ...
    for (size_t Index = 0; Index < CudaMemcpyFunctionCalls.size(); Index++) {
      ...
      BitCastInst *Destination =
          dyn_cast<BitCastInst>(MemCpy->getArgOperand(0));
      LoadInst *LoadDestination =
          dyn_cast<LoadInst>(Destination->getOperand(0));
      AllocaInst *AllocationDestination =
          dyn_cast<AllocaInst>(LoadDestination->getOperand(0));
      ...
      if (AllocatedVariableName == VariableName) {
        ...
        Value *BitCastedDestination =
            Builder.CreateBitCast(LoadedSource, Destination->getDestTy());
        ...
        Cloned->setArgOperand(
            3, ConstantInt::get(Type::getInt32Ty(MemCpy->getContext()), 3));
    ...
    for (BasicBlock::iterator CurrentInstruction = BB->begin(); CurrentInstruction != BB->end(); ++CurrentInstruction) {
      if (CallInst *FunctionCall = dyn_cast<CallInst>(CurrentInstruction)) {
        ...
        if (FunctionName == "cudaConfigureCall") {
          ...
          LoadInst *ThreadLoadInstr = dyn_cast_or_null<LoadInst>(FunctionCall->getArgOperand(2)); // 2 Thread X oluyor

          Configurations.ConfigurationCall = FunctionCall;
          GetElementPtrInst *GEPS = dyn_cast_or_null<GetElementPtrInst>(ThreadLoadInstr->getPointerOperand()); // Grid {i64, i32} şekilde bir struct.
          AllocaInst *ThreadAlloca = dyn_cast_or_null<AllocaInst>(GEPS->getPointerOperand()); // Grid'in alloca instruction'ı
          for (Use &U : ThreadAlloca->uses()) {
            User *User = U.getUser();
            BitCastInst *Casted = dyn_cast_or_null<BitCastInst>(User);
            if (Casted != nullptr) {
              Instruction *TmpInstruction = Casted;
          ...
          ThreadLoadInstr = dyn_cast_or_null<LoadInst>(FunctionCall->getArgOperand(0)); // 2 Thread X oluyor

          Configurations.ConfigurationCall = FunctionCall;
          GEPS = dyn_cast_or_null<GetElementPtrInst>(ThreadLoadInstr->getPointerOperand()); // Grid {i64, i32} şekilde bir struct.
          ThreadAlloca = dyn_cast_or_null<AllocaInst>(GEPS->getPointerOperand()); // Grid'in alloca instruction'ı
          for (Use &U : ThreadAlloca->uses()) {
            User *User = U.getUser();
            BitCastInst *Casted = dyn_cast_or_null<BitCastInst>(User);
            if (Casted != nullptr) {
  ...
  void parseOutput(std::vector<CallInst *> CudaMallocFunctionCalls,
                   Output *SingleOutput) {
  ...
  void
  createAndAllocateVariableAndreMemCpy(IRBuilder<> Builder,
                                       Output *OutputToReplicate,
                                       Instruction &LastInstructionOfPrevBB,
                                       FunctionCallee CudaMemcpy, bool IsLoop) {
    LLVMContext &Context = LastInstructionOfPrevBB.getContext();
    Type *Int32Type = Type::getInt32Ty(Context);
    Value *Three32Bit = ConstantInt::get(Int32Type, 3);
    AllocaInst *OutputToReplicateAllocation =
        OutputToReplicate->OutputAllocation;
    ...
    for (int Replication = 0; Replication < NumberOfReplication - 1;
         Replication++) {
      Builder.SetInsertPoint(OutputToReplicateAllocation->getNextNode());
      Instruction *NewAllocated =
          Builder.CreateAlloca(OutputType, nullptr, OutputToReplicate->Name);
      Instruction *ClonedMalloc = OutputToReplicate->MallocInstruction->clone();
      CallInst *Cloned = dyn_cast<CallInst>(ClonedMalloc);
      Value *BitcastedCloned =
          Builder.CreateBitCast(NewAllocated, DestinationType);
      ...
      BitcastedCloned =
          Builder.CreateBitCast(Builder.CreateLoad(NewAllocated),
                                DestinationType->getPointerElementType());
      Value *LoadedOutput = Builder.CreateLoad(OutputToReplicateAllocation);
      Value *BitcastedOutput = Builder.CreateBitCast(
          LoadedOutput, DestinationType->getPointerElementType());
      Builder.CreateCall(CudaMemcpy,
                         {BitcastedCloned, BitcastedOutput, Size, Three32Bit});
      OutputToReplicate->Replications.push_back(NewAllocated);
    }
  }

  Function *createRevisted(Module &M, std::string FunctionName,
                           FunctionCallee RevistedFunctionCallee) {
    ...
    while (Args != RevistedFunction->arg_end()) {
      ...
      if (ArgumanType == Int32PtrType || ArgumanType == Int32Type ||
          ArgumanType == FloatType)
        SizeParameter = 4;
      else
        SizeParameter = 8; // Diğerleri pointer olduğu için herhalde. Char*,
                           // float*, int* aynı çıktı.
      MaybeAlign ArgumanAlign = MaybeAlign(SizeParameter);
      AllocaInst *ArgumanPointer =
          Builder.CreateAlloca(ArgumanType, nullptr, "Arguman.addr");
      ...
      Value *BitcastParameter =
          Builder.CreateBitCast(ArgumanPointer, Int8PtrType);
    ...
    Value *FunctionNameString = Builder.CreateGlobalStringPtr(FunctionName);
    Builder.CreateCall(CudaRegisterFunction2,
                       {FirstInstruction->getOperand(0),
                        Builder.CreateBitCast(RevistedFunction, Int8PtrType),
                        FunctionNameString, FunctionNameString,
                        ConstantInt::get(Int32Type, -1), Null8bitPtr,
                        Null8bitPtr, Null8bitPtr, Null8bitPtr, Null32bitPtr});

    return RevistedFunction;
  }

  FunctionType *getTheNewKernelType(FunctionType *OriginalFunctionType,
    ...
    for (size_t ArgIndex = 0; ArgIndex < NumberOfArg; ArgIndex++) {
      NewKernelTypes.push_back(
          OriginalFunctionType->getFunctionParamType(ArgIndex));
    }

    for (Output SingleOutput : Outputs) {
      Type *SingleOutputType = SingleOutput.OutputType;
      for (int TempIndex = 0; TempIndex < NumberOfReplication - 1;
           TempIndex++) {
        NewKernelTypes.push_back(SingleOutputType);
      }
    }

    for (int DimensionIndex = 0; DimensionIndex < NumberofDimension;
         DimensionIndex++) {
      NewKernelTypes.push_back(Int32Type);
    }

    NewKernelFunctionType = FunctionType::get(
        OriginalFunctionType->getReturnType(), NewKernelTypes, true);
          ...
          if (CallInst *FunctionCall = dyn_cast<CallInst>(CurrentInstruction)) {
            ...
            if (isReplicate(FunctionCall)) {
              ...
              for (size_t Index = 0; Index < Outputs.size(); Index++) {
                ...
                createAndAllocateVariableAndreMemCpy(Builder, &SingleOutput,
                                                     LastInstructionOfPrevBB,
                                                     CudaMemCpy, IsLoop);
              ...
              NewArgs.push_back(Builder.CreateMul(Configurations.GridX, Configurations.ThreadX));

              NewArgs.push_back(Builder.CreateMul(Configurations.GridY, Configurations.ThreadY));

              FunctionType *NewKernelType = getTheNewKernelType(
                  FunctionCall->getFunctionType(), OutputsToBeReplicated);
              std::string NewKernelName = FunctionName.str() + RevisitedSuffix;
              errs() << NewKernelName << "\n";
              FunctionCallee NewKernelAsCallee =
                  M.getOrInsertFunction(NewKernelName, NewKernelType);
              Instruction *NewFunctionCall =
                  Builder.CreateCall(NewKernelAsCallee, NewArgs);
              ...
              Instruction *FirstInstruction =
                  dyn_cast<Instruction>(PrevBB->begin());
              FirstInstruction = dyn_cast<Instruction>(NextBB->begin());

              for (size_t OutputIndex = 0;
                   OutputIndex < OutputsToBeReplicated.size(); OutputIndex++) {
                ...
                Instruction *ConfigureCheck = dyn_cast<Instruction>(
                    Builder.CreateICmpNE(ClonedConfigure, One32Bit));

                Instruction *FirstInstructionOfNextBB =
                    SplitBlockAndInsertIfThen(
                        ConfigureCheck, ConfigureCheck->getNextNode(), false);
                ...
                std::string MajorityFunctionName =
                    "majorityFunction" +
                    std::to_string(OutputType->getTypeID());
                Function *MajorityFunction =
                    M.getFunction(MajorityFunctionName);
                if (MajorityFunction == nullptr)
                  MajorityFunction = createMajorityVoting(
                      M, dyn_cast<PointerType>(OutputType));

                Value *OrijinalOutput = CurrentOutput.OutputAllocation;
                Value *FirstReplicationOutput =
                    CurrentOutput.Replications.at(0);
                Value *SecondReplicationOutput =
                    CurrentOutput.Replications.at(1);
                Value *SizeOfOutput =
                    CurrentOutput.MallocInstruction->getArgOperand(1);

                Value *LoadedOrijinalOutput =
                    Builder.CreateLoad(OrijinalOutput);
                Value *LoadedFirstReplicationOutput =
                    Builder.CreateLoad(FirstReplicationOutput);
                Value *LoadedSecondReplicationOutput =
                    Builder.CreateLoad(SecondReplicationOutput);
                // Value* LoadedSizeOfOutput = Builder.CreateLoad(SizeOfOutput);

                Builder.CreateCall(
                    MajorityFunction,
                    {LoadedOrijinalOutput, LoadedFirstReplicationOutput,
                     LoadedSecondReplicationOutput, SizeOfOutput});
              }

              /*
                            createRevisted(Module &M, std::string FunctionName,
                 FunctionCallee RevistedFunctionCallee)

                           for(int ReplicationIndex = 1; ReplicationIndex <
                 NumberOfReplication; ReplicationIndex++){ CallInst*
                 ClonedConfigureCall =
                 dyn_cast<CallInst>(ConfigurationCall->clone());
                              ClonedConfigureCall->insertBefore(FirstInstructionOfNextBB);

                                Builder.SetInsertPoint(ClonedConfigureCall->getNextNode());
                                Instruction* ConfgurationCheck =
                 dyn_cast<Instruction>(Builder.CreateICmpNE(ClonedConfigureCall,
                 One32Bit)); Instruction* NewBasicBlockFirstInstruction =
                 SplitBlockAndInsertIfThen(ConfgurationCheck,
                 ConfgurationCheck->getNextNode(), false); errs() <<
                 *NewBasicBlockFirstInstruction << "\n";

                                int NumberOfArg =
                 FunctionCall->getNumArgOperands(); std::vector<Value *>
                 ArgsOfReplicationFunction; for(int ArgIndex = 0; ArgIndex <
                 NumberOfArg - 1; ArgIndex++){ // Outputu çıkartıyoruz Value*
                 Arg = FunctionCall->getArgOperand(ArgIndex); if(Instruction*
                 ArgAsInstruction = dyn_cast<Instruction>(Arg)){ Instruction*
                 ClonedArg = ArgAsInstruction->clone();
                 ...
                 OutputsToBeReplicated.size(); OutputIndex++){ Instruction*
                 NewOutput =
                 OutputsToBeReplicated.at(OutputIndex).Replications.at(ReplicationIndex-1);
                                  ArgsOfReplicationFunction.push_back(Builder.CreateLoad(NewOutput));
                                }

                                Builder.CreateCall(FunctionToReplicate,
                 ArgsOfReplicationFunction);
                                }
