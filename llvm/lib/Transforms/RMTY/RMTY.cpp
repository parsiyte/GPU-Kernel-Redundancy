...
struct RMTYDevice : public ModulePass {
  ...
  void changeXID(Function *Kernel, Value* NewCreatedCall, Value *ToBeChange, IRBuilder<> Builder, Value* OriginalBased) {
    for (Function::iterator BB = Kernel->begin(); BB != Kernel->end(); ++BB) {
      for (BasicBlock::iterator CurrentInstruction = BB->begin(); CurrentInstruction != BB->end(); ++CurrentInstruction) {
        if (CallInst *FunctionCall = dyn_cast<CallInst>(CurrentInstruction)) {
          ...
          */if (FunctionName == "llvm.nvvm.read.ptx.sreg.tid.x") {
          ...
          }else if (FunctionName == "llvm.nvvm.read.ptx.sreg.ntid.x") {
            FunctionCall->replaceAllUsesWith(OriginalBased);
          } 
        }
      }
    }
  ...
  void changeYID(Function *Kernel, Value* NewCreatedCall, Value *ToBeChange, IRBuilder<> Builder, Value* OriginalBased) {
    for (Function::iterator BB = Kernel->begin(); BB != Kernel->end(); ++BB) {
      for (BasicBlock::iterator CurrentInstruction = BB->begin(); CurrentInstruction != BB->end(); ++CurrentInstruction) {
        if (CallInst *FunctionCall = dyn_cast<CallInst>(CurrentInstruction)) {
          if(FunctionCall == NewCreatedCall)
          ...
          */if (FunctionName == "llvm.nvvm.read.ptx.sreg.tid.y") {
            FunctionCall->replaceAllUsesWith(Builder.CreateLoad(ToBeChange));
          }else if (FunctionName == "llvm.nvvm.read.ptx.sreg.ntid.y") {
            FunctionCall->replaceAllUsesWith(OriginalBased);
          } 
        }
      }
  ...
  bool runOnModule(Module &M) override {
     ...
     for(unsigned int Index = 0; Index < Annotations->getNumOperands(); Index++){
       ...
       Instruction& LastInstruction = FirstBB->front();
        ...
        Value* ThreadIDYCall = Builder.CreateCall(ThreadIDY);
        /*
        if(BlockIDY != nullptr){
...
*/
         changeYID(NewKernelFunction,ThreadIDYCall,BlockIdaddr2, Builder, Builder.CreateLoad(OriginalBaseddr));
